...
function fnChangePassWd() {
	...
	var param = {'passWd' : $('#passWd').val(), 'newPassWd': $('#newPassWd').val(), 'newPassWdChk': $('#newPassWdChk').val()};
   	var url = '/usr/member/memberChangePassword';
    $.ajax({
       type :"POST",
       ...
       success:function(result){
     	  if(result){
               if(result.code == "0000") {//성공
            	   fnModalOpen('modaltest2', 'chkPassSuccess', 'location.href = "/usr/member/memberlogOut"');
               }else{//실패
...
//취소버튼 클릭 시
function fnCancel(){
	fnModalOpen('modaltest1', 'PassWdChkCancel', 'location.href="/main"');
}
...
<div class="log-wrap">
    <div class="inner">
            ...
            <div class="box-gray">
                ...
                <div class="input-wrap">
                    ...
                    <div class="row-btns">
                        ...
                        <span class="col">
                            <a href="javascript:;" onclick="fnValidation();" class="btn-red-lg">변경</a>
                        </span>
                    </div>
                </div>
            </div>
        ...
        <p class="comment">
            <em>비밀번호 변경 시, 문제가 발생할 경우 <br class="mo-br">‘서비스지원팀’</em> 으로 연락 바랍니다. <br class="mo-br">(담당자 : <em>서정일</em> jiseo@kr.canon)
         </p>
    </div>
</div>

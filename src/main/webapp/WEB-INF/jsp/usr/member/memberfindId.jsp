...
function fnFindId() {
	...
	var param = {'name' : $('#IdFindUserName').val(), 'email': $('#IdFindUserEmail').val()};
   	var url = '/usr/member/memberfindIdAjax';
   	$.ajax({
        type :"POST",
        ...
        success:function(result){
      	  if(result){
                if(result.code == "0000") {//성공
             	   fnModalOpen('modaltest2', 'idFindUserEmail', 'location.href = "/usr/member/memberLogin"');
                }else{//실패
...
function fnFindPw() {
	...
	var param = {'name' : $('#pwFindUserName').val(), 'id' : $('#pwFindUserId').val(), 'email': $('#pwFindUserEmail').val()};
   	var url = '/usr/member/memberfindPwAjax';
   	$.ajax({
        type :"POST",
        contentType: 'application/json;charset=UTF-8',
        url : url,
        ...
        success:function(result){
      	  if(result){
                if(result.code == "0000") {//성공
             	   fnModalOpen('modaltest2', 'pwFindUserEmail', 'location.href = "/usr/member/memberLogin"');
                }else{//실패
